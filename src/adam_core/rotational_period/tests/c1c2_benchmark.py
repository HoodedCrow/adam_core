...
def mm81_data():
    ...
    inputs = cache_or_query_rotational_period_inputs(object_id, "X05", obstime, "data/rot_period_inputs.parquet")
...
def test_initial_orbit_fit_benchmark(benchmark, mm81_data):
    ...
    result = benchmark(run_complete_fourier, object_id, obs_table, obstime, inputs, kind=None)
    assert result.object_id[0].as_py() == object_id, f"Wrong object_id '{result.object_id[0].as_py()}'"
    assert result.num_obs[0].as_py() == 390, f"Num obs is {result.num_obs[0]}, expected 390"
    def check_num(actual, expected, tolerance, message):
        assert abs(actual - expected) < tolerance, f"{message} is {actual}, expected {expected}"
